...
@pytest.mark.parametrize(
    "products",
    ["1m/1_3as/1m", ["1m", "1_3as", "1m"]],
)
def test_products_query_separately_and_preserve_source_metadata(monkeypatch, products):
    item = _item(
        "Project tile",
    ...
    mod = tnm.TheNationalMap(
        src_region=SAMPLE_REGION, products=products, use_cache=False
    )
    assert mod.products == ["1m", "1_3as"]
    mod.run()
