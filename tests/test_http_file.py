...
def test_encoded_range_is_rejected_before_body_read():
    session = Session()
    response = Response(
        206, {"Content-Range": "bytes 0-2/10", "Content-Encoding": "gzip"}, b"012"
    )
    session.get_response = response
    with HttpFile("https://example.test/archive.zip", session) as remote:
        with pytest.raises(OSError, match="encoded byte range"):
            remote.read(3)
        assert remote.tell() == 0
    assert response.reads == 0
    assert response.closed
...
def test_successful_responses_are_closed():
    session = Session()
    response = Response(206, {"Content-Range": "bytes 0-2/10"}, b"012")
    session.get_response = response
    with HttpFile("https://example.test/archive.zip", session) as remote:
        assert session.head_response.closed
        assert remote.read(3) == b"012"
        assert response.closed
