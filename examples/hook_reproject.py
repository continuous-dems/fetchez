...
class Reproject(FetchHook):
    ...
    def run(self, entries):
        ...
        for mod, entry in entries:
            ...
            source_crs = entry.get(
                "crs"
            )  # context check (this may have been added by the module)
            ...
            try:
                ...
                # We use gdal.Warp
                logger.info(
                    f"Reprojecting {os.path.basename(src_path)} to {self.target_crs}..."
                )

                warp_opts = gdal.WarpOptions(
                    dstSRS=self.target_crs, srcSRS=source_crs, format="GTiff"
                )
