...
class EarthAccess(FetchModule):
    ...
    def run(self):
        try:
            ...
            logger.error(
                "Module 'earthaccess' requires the library. Run: pip install earthaccess"
            )
            return
        ...
        try:
            ...
            if not auth.authenticated:
                logger.warning(
                    "EarthAccess: Not authenticated. Public data only, or prompt incoming."
                )
        ...
        logger.info(
            f"Searching CMR (via earthaccess) for {self.short_name or self.concept_id}..."
        )
